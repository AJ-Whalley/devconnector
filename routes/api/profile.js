...
router.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        profile.experience.splice(
          profile.experience.findIndex(exp => exp._id === req.params._id),
          1
        );
    ...
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        profile.education.splice(
          profile.education.findIndex(exp => exp._id === req.params._id),
          1
        );
        profile.save().then(res.json(profile));
      })
...
router.delete(
  ...
  (req, res) => {
    Profile.findOneAndRemove({ user: req.user.id })
      ...
      .catch(err => res.status(404).json({ User: "Could not find user" }));
  }
);
